...
public class IGUtils {
    public static boolean canMobBypassIllusions(LivingEntity entity) {
        ...
        boolean resistNormal = entity.getAttributeValue(AttributeRegistry.SPELL_RESIST) > 1.2;
        return resistEvocation || resistNormal || entity.getType().is(ModTags.ILLUSION_IMMUNE_ENTITIES);
    }

    public static boolean canBypassIllusions(LivingEntity entity, double illusionistSpellPower) {
        double resist = (1 - entity.getAttributeValue(AttributeRegistry.EVOCATION_MAGIC_RESIST)) + ((1 - entity.getAttributeValue(AttributeRegistry.SPELL_RESIST) * 0.5));
